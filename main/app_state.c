...
static void handle_key(app_state_t *s, const app_event_t *ev, uint64_t now_ms,
    ...
    // 锁定态(真锁,v0.2.2):除长按 OK 解锁外全部按键吞掉,见下方 locked 门禁。
    const bool screen_was_off = !s->screen_on;

    // 锁定态(v0.2.2 语义变更 = 真锁):长按 OK 解锁亮屏;其余按键全部忽略。
    // 旧语义(2026-08-28)是"省电模式非输入锁"——锁定后按键照常执行(口袋
    // 盲操说话/回车/清空)。真机反馈(2026-09-08):锁屏后音量-仍把对话框文字
    // 发出去(回车),口袋误触即误发 —— 手动锁屏改为真锁:只有解锁一条路,
    // 不回车上行、不清空、不开录音、不唤醒。自动息屏(非 locked)保留原
    // 盲操语义(按键唤醒+照常执行),两档自此分离。
    if (s->locked) {
        ...
        // 真锁:非解锁按键直接吞掉 —— 不落状态机(不回车上行/不清空/不开
        // 录音/不唤醒),防口袋误触发送。
        return;
    } else {
        if (screen_was_off) {
